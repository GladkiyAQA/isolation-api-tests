...
class OperationsHTTPTestClient(HTTPTestClient):
    ...
    Этот клиент работает с реальным operations-service,
    поэтому не использует сценарии и RequestContext.
    Он вызывает реальные HTTP-эндпоинты по контракту
    и возвращает типизированные результаты тестовому слою.
    """
